...
Future<String> getUserInfo(String choice) async {
  ...
  Future<List<Map<String, dynamic>>> queryEventsforDay(int day, int month, int year) async {
    Database db = await database;
  ...
  Future<void> setCaloriesForDay(int day, int month, int year, int newCalories) async {
    final events = await queryEventsforDay(day, month, year);
    for (final event in events) {
  ...
  Future<int> updateCalories(Event event, int day, int month, int year, int newCalories) async {
    final db = await database;
    return await db.update('events', {'totalCalories': newCalories},
        where: 'day = ? AND month = ? AND year = ?', whereArgs: [day, month, year]);
  }
  ...
  Future<double> getCaloriesBurnedForWorkout(String workoutName, String workoutMuscle, int day, int month, int year) async {
    double caloriesBurned = 0;
    final events = await queryWorkout(workoutName, workoutMuscle, day, month, year);
    if (events.isNotEmpty) {
      for (final event in events) {
  ...
  Future<void> setCaloriesBurnedForWorkout(
      String workoutName, String workoutMuscle, int day, int month, int year, double newCaloriesBurned) async {
    final events = await queryEventsforDay(day, month, year);
    for (final event in events) {
      await updateCaloriesBurned(Event.fromMap(event), workoutName, workoutMuscle, day, month, year, newCaloriesBurned);
    }
  }

  // Query workout by day
  Future<List<Map<String, dynamic>>> queryWorkout(String workoutName, String workoutMuscle, int day, int month, int year) async {
    Database db = await database;
    return await db.query('events',
        where: 'workoutName = ? AND workoutMuscle = ? AND day = ? AND month = ? AND year = ?',
        whereArgs: [workoutName, workoutMuscle, day, month, year]);
  }

  // Update calories burned for a workout
  Future<int> updateCaloriesBurned(Event event, String workoutName, String workoutMuscle, int day, int month, int year, double newCaloriesBurned) async {
    final db = await database;
